...
import static io.github.fiftieshousewife.recipes.CommonsLoggingToSlf4j.findCommonsLogField;
import static io.github.fiftieshousewife.recipes.CommonsLoggingToSlf4j.removeField;
import static io.github.fiftieshousewife.recipes.CommonsLoggingToSlf4j.renameReferences;
...
@NullMarked
class CommonsLoggingToSlf4jVisitor extends JavaIsoVisitor<ExecutionContext> {
    ...
    private boolean shouldConvert(final J.ClassDeclaration classDecl) {
        return !AddLombokSlf4jAnnotation.hasLombokLoggingAnnotation(classDecl)
                && findCommonsLogField(classDecl).isPresent()
    ...
    private J.ClassDeclaration convertToSlf4j(final J.ClassDeclaration classDecl) {
        ...
        final J.ClassDeclaration fatalRenamed =
                (J.ClassDeclaration) new CommonsFatalRenameVisitor().visitNonNull(annotated, 0);

        return findCommonsLogField(fatalRenamed)
                .map(field -> removeField(renameReferences(fatalRenamed, field.name()), field.varDecl()))
                .orElse(fatalRenamed);
    }
}
