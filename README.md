...
| **Multi-module Lombok safety check** | None. The recipe runs against every source file regardless of whether the surrounding module actually has Lombok available. In a multi-module project where Lombok is a dependency of only some modules, the recipe inserts `@Slf4j` in modules where `lombok.extern.slf4j.Slf4j` won't resolve, leaving them with code that no longer compiles. | **`requireLombokOnClasspath` option** (and `*NoDeps` recipe variants). Before inserting `@Slf4j` in a source file, the recipe checks whether `lombok.extern.slf4j.Slf4j` is on that module's classpath; if not, the file is left alone. Modules without Lombok stay untouched rather than being rewritten into uncompilable code. |
...
### `StringFormatInLogCall`

Rewrites SLF4J calls whose single argument is `String.format(...)` to the parameterised form:

```java
log.info(String.format("user %s did %s", userId, action));
```
...
```kotlin
rewrite {
    activeRecipe("io.github.fiftieshousewife.cleanlogging.StringFormatInLogCall")
}
```

The printf specifiers (`%s` / `%d` / `%f` / `%n` / `%%` / width + precision flags) are converted to SLF4J `{}` placeholders by the same converter that powers `SystemOutToSlf4j`'s `printf` translation.

| Skipped when | Reason |
| --- | --- |
| Receiver isn't named `log` | Heuristic guard — Lombok convention is `log`. |
| Method name isn't an SLF4J level | Only `trace`/`debug`/`info`/`warn`/`error` are touched. |
| Inner call isn't `java.lang.String.format(...)` | The `String.format(Locale, ...)` overload is intentionally skipped to preserve explicit locale handling. |
